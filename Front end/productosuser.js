  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      let resultado = JSON.parse(this.responseText);
            ...
            " <td><input type='number' id='quantityByName" + resultado.id_producto + "' min='1' max='" + resultado.existencia + "' placeholder='Qty' class='form-control entrada'></td> </td>" +
          "<td> <button class='btn btn-success' onclick='addByName(" + resultado.id_producto + ")'>ADD</button></td>" +
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      for (let i = 0; i < list.length; i++) {
        main += "<tr><th scope='row' style='text-align:center'>" + list[i].id_producto + "</th><td>" + list[i].descripccion + "</td>"
          ...
          " <td><input type='number' id='quantityList2" + list[i].id_producto + "' min='1' max='" + list[i].existencia + "' placeholder='Qty' class='form-control entrada'></td> </td>" +
          "<td> <button class='btn btn-success' onclick='addList2(" + list[i].id_producto + ")'>ADD</button></td>" +
          "<td> <button class='btn btn-danger' onclick='resetCar(" + list[i].id_producto + ")'>DELETE</button></td></tr>";
      }
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      xhr_1.onreadystatechange = function () {
        ...
        if (xhr_1.readyState === 4 && xhr_1.status === 200) {
          ...
          console.log("tipo recibido",resultado.tipo);
      ...
      var data = JSON.stringify({
        ...
        "monedero": wallet, "tipo": object.tipo
      });
      xhr_1.send(data);
...
function createTicket(promocion1, promocion2, subtotal, total) {
  ...
  xhr.send(data);
  alert("Order ID: " + JSON.parse(localStorage.getItem("id_compra")) + "\nSave your Order ID");
}
